! Full K space sample with err indicator
   subroutine KPP_run(N, u_hat, lam, eps, tau, theta, dt, t, Amp, y, localN, local_0_start, myrank, nprocs, ierr)
       include'mpif.h'
...
!    do while( t .le. tmax)
 ...
 !       write(*,*)'hello world'
        call sparse_mpi_ComputePlane(pi, cost, dt, N, C, Amp, lam,  theta, eps, u_hat, mpirhs_hat, localN, &
...
!             call zcopy((N/2 + 1)*N, testu_hat, 1, UP_hat(:, niter/nc), 1)
!            end if
!
!        if(mod(exiter, 100) .eq. 0) then
!          call zcopy((N/2 + 1)*N, testu_hat, 1, conv_hat, 1)
!        else
!         call zaxpy((N/2 + 1)*N, dcmplx(1.0D+0,0.0D+0), testu_hat, 1, &
!                    conv_hat, 1)
!        end if
!
...
!!       end if
 !      end if  
    ...
    if(myrank .eq. 0) then
       do j=1,N
          do i=1,N/2+1
            write(*,*) 'test:',myrank,  testu_hat(i,j)/(N*N)
          end do 
        end do
       end if
