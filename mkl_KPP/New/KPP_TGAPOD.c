...
/*****************************collect Coarse Grid PW solution ***********************/
       if(t + CoardT > T)
           ...
           CoarN = CoardT/Cdt;
       gatherU1 = calloc(N1*CoarN, sizeof(*gatherU1));
       S1 = calloc(CoarN, sizeof(*S1));
	   B1 = calloc(localN*N*N*N, sizeof(*B1));
       
       for(k=0; k< CoarN; k++){
          t = t + Cdt;
          KPP_ComputePlane(N, t, Cdt, C, Amp, lam, theta, eps, cost, Coaru_hat, myrank, nprocs,localN, local_0_start,  comm1);
          if(k%CoarIntval ==0)
	         zcopy_(&N1,  Coaru_hat, &ONE, &gatherU1[(k/CoarIntval)* N1], &ONE);
       }
	   KPP_Build_FFTMatrix1(N, CoarN, localN, gatherU1, tmp4svd_hat1, comm1);
       KPP_mpi_svd(CoarN, N, localN, tmp4svd_hat1, S1, B1, nprocs, myrank);
       nPOD1 = KPP_GetPODNumber(S1, CoarN, gamma2);
/******************************Build POD Matrix *************************************/
       free(S);   free(B);   free(gatherU1);       
