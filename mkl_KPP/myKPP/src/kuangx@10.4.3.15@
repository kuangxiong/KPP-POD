...
{  
    ...
    complex value, *a_imag, alpha, beta, *tmpU, tmpvalue, *tmpdata, *testU, *error;
...
//            testU[i+j*ncup]=U[i+j*ncup]*sqrt(tmpS[i]);
    ...
    zgemm_("N", "T", &tmpM, &tmpN, &tmpN, &alpha, tmpU, &tmpM , a_imag, &tmpN,  &beta, 
            testU, &tmpM);
    ...
    if(myid==0){
        for(i=0; i< ncup; i++)
            for(j=0; j< localN*N; j++)
                if(creal(tmpdata[i*localN*N+j])>0.00000001||cimag(tmpdata[i*localN*N+j])>0.0000001)
                    printf("testEV:%e&%e\t%e&%e\n", tmpdata[i*localN*N+j], error[i*localN*N+j]);
