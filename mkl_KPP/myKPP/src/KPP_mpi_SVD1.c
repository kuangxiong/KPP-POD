...
void
KPP_mpi_svd1(int ncup, int N, int localN, complex *data, double *S, complex *U, int nprocs, int myid)
{  
	int context, i, j, k, info, lwork, M, tmpN;
	int descA[9], nprow, npcol,myrow, mycol, mU, nb;
	int  descU[9], descV[9], mA, ONE=1;
	double  wkopt, *rwork, rwkopt, *V;
    complex *work;
...
//	}
    ...
    descV[0] = 1; descV[1] = 0; descV[2] =M; descV[3]= M; descV[4]= 1; 
    descV[5]= 1; descV[6] = 0; descV[7] = 0; descV[8]= mU;
	/********************************************************************* */
	lwork= -1;
	info=0;
