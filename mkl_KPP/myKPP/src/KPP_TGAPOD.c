 ...
 */
/* ============================================================================*/
void KPP_TGAPOD(int N, DOUBLE lam, DOUBLE eps, DOUBLE tau, DOUBLE theta, DOUBLE dt, DOUBLE t, 
...
{     
	 ...
	 complex *u_hat, *UP_hat, *gatherU, *gatherU1, testu_hat[N*(N/2+1)], *tmp4svd_hat,
			 *B, *B1, *BN1, *BN, *PODu0, *Firm, *Varm, *tmpFirm, *PODu, *CPODu, *tmpu_hat, *tmpu1_hat,
			 *CtmpV, *BNlap, *TMat, *NotTMat, alpha, tmpvalue, beta, *mixMat, *err_hat, *tmpPODu0;
     fftw_plan p1, p2;
     int i, k, j, niter = 0, ONE=1, N1, N2, nPOD, nPOD1, nPOD2,  tmpN, tmpN1, flag, tmpnPOD;
	 FILE *fp, *fp1;

	 fp = fopen("TGAPODerr.text","w");
	 fp1 = fopen("TGPODerrind.text","w");
     alloc_local = fftw_mpi_local_size_2d(N, N/2+1, comm1, &localN, &local_0_start);
     u_hat = calloc(localN*(N/2+1), sizeof(*u_hat));
     tmpu_hat = calloc(localN*(N/2+1), sizeof(*tmpu_hat));
     tmpu1_hat = calloc(localN*(N/2+1), sizeof(*tmpu1_hat));
...
/********************get initial POD solution****************/
    ...
    tmpPODu0 = calloc(nPOD, sizeof(*tmpPODu0));
	for(i=0; i< nPOD; i++)
...
//    for(i=0; i< nPOD; i++)
//        printf("PODu0:%f&%f\n", PODu0[i]);
//	KPP_BuildPODMatrix1(nPOD, N, localN, local_0_start, alloc_local, eps,theta, 
//                     lam, C, Amp, BN1, NotTMat, TMat, Firm, Varm, cosx, sinx, 
//                     myrank, nprocs, comm1);
	tmpN = localN*(N/2+1);
...
//				uPOD[i*N+j] = RtmpV[i*2*localN*(N/2+1)+j]/(N*N);  
		/***************************computing TGAPOD algorithm  err indicator*********************/
#endif
//        if(myrank==0)
//            printf("i am here\n");
      if(flag % 200 ==0)
        {
    	    zcopy_(&N1, CPODu, &ONE, tmpu_hat, &ONE);
//    	    zcopy_(&N1, u_hat, &ONE, tmpu_hat, &ONE);
            flag ==0;
            KPP_ComputePlane(N, t, 100*dt, C, Amp, lam, theta, eps, cost, tmpu_hat, myrank, nprocs, localN, local_0_start,  comm1);
            //**************get initilal POD solution***********************/
            KPP_GetInitialPOD(N, localN, nPOD, tmpu_hat, BN1, tmpPODu0, myrank, nprocs);
            for(i=0; i< nPOD; i++)
                tmpPODu0[i]=creal(tmpPODu0[i]);
//            if(myrank==0)
//                for(i=0; i< nPOD; i++)
//                    printf("tmpPODu0:%f\t%f\n", tmpPODu0[i]);
            //***************transform POD solution to PW solution*********/
            zgemv_("N", &tmpN, &nPOD, &tmpvalue, BN, &tmpN, tmpPODu0, &ONE, &beta, tmpu1_hat, &ONE);
            for(i=0; i< localN ; i++)
                for(j=0; j< N/2+1; j++)
                    tmpu_hat[i*(N/2+1)+j] = tmpu1_hat[i*(N/2+1)+j]-tmpu_hat[i*(N/2+1)+j];
//                    tmpu_hat[i*(N/2+1)+j] = CPODu[i*(N/2+1)+j]-tmpu_hat[i*(N/2+1)+j];
            localerr = dnrm2_(&tmpN1, tmpu_hat, &ONE);
            localerr = localerr * localerr;
            MPI_Allreduce(&localerr, &err, ONE, MPI_DOUBLE, MPI_SUM, comm1);
            errind = sqrt(err);  
            if(myrank==0)
...
//          KPP_mpi_svd(nPOD+nPOD1, N, localN, mixMat, S, B, nprocs, myrank); 
            ...
            PODu = calloc(nPOD, sizeof(*PODu));
            /*****************************************************************
             * Attention:since eigen decomposition is descended , so the last 
             * eigenvector is more  important
             * ***************************************************************/
            for(i=0; i< nPOD; i++){
//                zcopy_(&N2, &B[i*N2], &ONE, &BN1[i*N2], &ONE);
            ...
            /*******************************************************************************
             * Attention!!!,in theory, PODu is a real vector, however, the imaginary
             * part of PODu is a small number in here, so we set the imaginary of PODu is 0 
             * *****************************************************************************/
            for(i=0; i< nPOD; i++)
                PODu[i] = creal(PODu[i]);
/***********************************test*****************************************/
