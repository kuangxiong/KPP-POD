...
void
KPP_mpi_ev1(int ncup, int N, int localN, complex *data, double *S, complex *U, int nprocs, int myid)
{  
	int context, i, j, k, info, lwork, M, tmpN;
	int descA[9], nprow, npcol,myrow, mycol, mU, nb, mA, ONE=1, tmpM, MM;
	double  *a, *work, wkopt , *tmpS;
    complex value, *a_imag, alpha, beta, *tmpU, *tmpU1, tmpvalue, *tmpdata, *testU, *error, valuetest;
    ...
    tmpU1 = calloc(ncup*localN*N, sizeof(*tmpU1));
    testU = calloc(ncup*localN*N, sizeof(*testU));
    error = calloc(ncup*localN*N, sizeof(*error));
	tmpdata = calloc(localN*N*ncup, sizeof(*tmpdata));
...
//            for(j=0; j< tmpM; j++)
//                U[i*tmpM+j] = (1.0/sqrt(tmpS[i])) * tmpU[i*tmpM+j];
        }
    }

    for(i=0; i< ncup; i++){
        if(tmpS[i]>0){
              value = sqrt(tmpS[i])+0*I;  
//            valuetest=(1.0/sqrt(tmpS[i])+0*I)*sqrt(tmpS[i]);
//            if(myid==0)
//            printf("value:%e&%e\t%e&%e\n", value, valuetest);
              zaxpy_(&tmpM, &value, &U[i*tmpM], &ONE, &tmpU1[i*tmpM], &ONE);
//            for(j=0; j< tmpM; j++)
//                tmpU1[i*tmpM+j] = sqrt(tmpS[i]) * U[i*tmpM+j];
        }
    }
//    if(myid==0)
//    for(i=0; i< ncup; i++ )
//        for(j=0; j< tmpM; j++)
//            if(tmpS[i]>0)
//            if(fabs(tmpU[i*tmpM+j]-tmpU1[i*tmpM+j])>1.0e-20){
//                value = (1.0/sqrt(tmpS[i]))*tmpU[i*tmpM+j];
//                valuetest = sqrt(tmpS[i])*value;
//                printf("error:%e&%e\t%e&%e\t%e\n",tmpU[i*tmpM+j], valuetest, fabs(tmpU[i*tmpM+j]-tmpU1[i*tmpM+j]));
//            }
     ...
     * **********************************************/
//    for(i=0; i< ncup; i++)
//        for(j=0; j< localN*N; j++)
//            testU[i+j*ncup]=U[i+j*ncup]*sqrt(tmpS[i]);
    
    zgemm_("N", "T", &tmpM, &tmpN, &tmpN, &alpha, tmpU1, &tmpM , a_imag, &tmpN,  &beta, 
            testU, &tmpM);
    
    for(i=0; i< ncup; i++)
        for(j=0; j< localN*N; j++)
            error[i*localN*N+j] = testU[i*localN*N+j]-tmpdata[i*localN*N+j];
    if(myid==0){
        printf("\n/*************************\n* EV error\n**************************/\n");
        for(i=0; i< ncup; i++)
            for(j=0; j< localN*N; j++)
                if(fabs(creal(error[i*localN*N+j]))>1.0e-12||fabs(cimag(error[i*localN*N+j]))>1.0e-12)
                    printf("%e&%e\t%e&%e\n", creal(tmpdata[i*localN*N+j]),cimag(tmpdata[i*localN*N+j]),
                            creal( error[i*localN*N+j]),cimag(error[i*localN*N+j])); 
    }
