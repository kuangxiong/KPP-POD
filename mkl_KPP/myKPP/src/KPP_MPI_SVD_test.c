...
//	printf("hello:%d\t%d\n", lwork, tmpN);
    ...
    if(myid==0){
        ...
        for(i=0; i< mA; i++)
        {    for(j=0; j< ncup; j++)
                if(fabs(creal(error[i*ncup+j]))>1.0e-16||fabs(cimag(error[i*ncup+j]))>1.0e-16)
                     printf("%e&%e\t%e&%e\n",creal(testMix[i*ncup+j]),cimag(testMix[i*ncup+j]), creal(error[i*ncup+j]), cimag(error[i*ncup+j]));
//            printf("\n");
        }
    }
    /****************************************************/
