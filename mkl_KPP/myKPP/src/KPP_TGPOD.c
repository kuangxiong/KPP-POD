...
{     
     ...
     fftw_plan p1, p2;
     int i, k, j, niter = 0, ONE=1, N1, N2, nPOD, nPOD1, nPOD2,  tmpN, tmpN1, CoarN, flag, 
         CoarIntval, tmpnPOD;
	 FILE *fp;
...
//	   B1 = calloc(localN*N*(CoarN/CoarIntval+1), sizeof(*B1));
       ...
       for(k=0; k<= CoarN; k++){
          ...
          if(k%CoarIntval ==0){
             ...
             if(myrank==0)
                 printf("123123123:%d\t%d\n",CoarN/CoarIntval+1, k/CoarIntval);
          }
       }
...
//       if(myrank==0)
//        for(i=0; i< nPOD; i++)
//            printf("hello1111:%d\t%f&%f\n",nPOD, PODu[i]);
    //**********************error******************//
    /*********transform POD solution to PW solution *************/
       zgemv_("N", &N1, &nPOD, &tmpvalue, BN, &N1, PODu, &ONE, &beta, CPODu, &ONE);
       /***********************************************
...
//                err_hat[i*(N/2+1)+j] =  u_hat[i*(N/2+1)+j];
       ...
       if(myrank==0){
           fprintf(fp, "%f\t%e\t%e\n", t, sqrt(err/Norm), sqrt(Norm));
//         printf("%f\t%e\t%e\n", t, sqrt(err), sqrt(Norm));
       }
      }
// /********************transform POD solution to PW solution *****************/
//   zgemv_("N", &N1, &nPOD, &tmpvalue, BN, &N1, PODu, &ONE, &beta, Coaru_hat, &ONE);
	 zcopy_(&N1,  CPODu, &ONE, Coaru_hat,  &ONE);
     }
	 free(S);
	 free(B);
	 free(CtmpV);
