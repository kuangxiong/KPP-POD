...
void
KPP_BuildPODMatrix1(int nPOD, int N, int localN, int local_0_start, int alloc_local, double eps, double theta, 
		...
		for(i=0; i< localN; i++){
			for(j=0; j< N/2+1; j++){
				if(index[i] < N/2+1){
					BNlap[k*(localN*(N/2+1))+i*(N/2+1)+j] = -4*PI*PI*(i*i+j*j)
						*BN[k*(localN*(N/2+1))+i*(N/2+1)+j];
				    NotTMat[k*localN*(N/2+1)+i*(N/2+1)+j] += eps * BNlap[k*localN*(N/2+1)+i*(N/2+1)+j];
				}
				else{
					BNlap[k*(localN*(N/2+1))+i*(N/2+1)+j] = -4*PI*PI*((index[i]-N)*(index[i]-N)+j*j) 
						*BN[k*(localN*(N/2+1))+i*(N/2+1)+j];
	...
	for(k=0; k< nPOD; k++){
		for(i=0; i< localN; i++){
			for(j=0; j< N/2; j++){
					...
					NotTMat[k*(localN*(N/2+1))+i*(N/2+1)+j] += 2*eps*lam*BNx[k*(localN*(N/2+1))+i*(N/2+1)+j]; 
                }
					BNx[k*(localN*(N/2+1))+i*(N/2+1)+N/2] = 0; 
                    NotTMat[k*localN*(N/2+1)+i*(N/2+1)+N/2] += 0;
			}
		}
/******************copy and symmetrize(different with zliu)*******************/
    KPP_Build_FFTMatrix1(N, nPOD, localN, BNx, tmphat1, comm);
...
/*******************compute eps*BN*BNlap+2*eps*lam*BN*BNx + CI*************************/
	for(k=0; k<nPOD; k++){
		Firmx[k*nPOD+k] = Firmx[k*nPOD+k] + C;
	    for(i=0; i< localN; i++)
            for(j=0; j<N/2+1; j++)
                NotTMat[k*localN*(N/2+1)+i*(N/2+1)+j] += C*BN[k*localN*(N/2+1)+i*(N/2+1)+j];
    }
...
//		fftw_execute_dft_r2c(pback, RtmpV, CtmpV);
		...
		for(i=0; i< localN; i++){
			for(j=0; j< N/2+1; j++){
				tmphat1[k*(localN*N)+i*N+j] = CtmpV[i*(N/2+1)+j];
				NotTMat[k*localN*(N/2+1)+i*(N/2+1)+j] += CtmpV[i*(N/2+1)+j];
...
//	}
	...
	for(k=0;k<nPOD; k++){
		...
		for(i=0; i< localN; i++){
			for(j=0; j< N/2+1; j++){
				tmphat1[k*(localN*N)+i*N+j] = CtmpV[i*(N/2+1)+j];
				Tmat[k*localN*(N/2+1)+i*(N/2+1)+j] = CtmpV[i*(N/2+1)+j];
			}
			for(j=N/2+1; j< N; j++){
				tmphat1[k*(localN*N)+i*N+j] = conj(CtmpV[i*(N/2+1)+N-j]);
			}
