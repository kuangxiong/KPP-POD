...
!     ncup =21 
     ...
     call zcopy((N/2 + 1)*N, u_hat, 1, UP_hat(:, 1), 1)
...
!        call zcopy((N/2 + 1)*N, u_hat, 1, UP_hat(:, niter/nc), 1)
        call zcopy((N/2 + 1)*N, u_hat, 1, UP_hat(:, niter), 1)
!      end if
!
...
!      do j=1, N
!          do i=1, N/2+1
!            write(*,'(3F10.6)'), u_hat(i,j)
!!            k=k+1
!          end do 
!       end do
!       write(*,*)"googd", uy_hat(N/2+1,N)

     print*, 'ncup', ncup, niter
...
!    write(*,*)'test' 
!    do i=1, N*N
!        write(*,'(3F10.6)'), u_hat1(i)
!     end do 
!     do i=1, N*N
!        write(*,'(3F10.6)(3F10.6)'), u_hat1(i), BN1(i, 1)
!     end do 
     
     do i=1, m
        write(*,'(3F12.6)'), a_hat(i)
     end do 
...
!     end do
 ...
 !    do i=1, N
 !       do j=1, N/2+1
 !           write(*,'(3F10.6)'), lam*BBN((i-1)*(N/2+1)+j, 1)
 !        end do
 !    end do
...
!     do i=1, N
!        do j=1, N
!            write(*,'(3F10.6)'), temp_hat1((i-1)*N+j, 1)
!         end do
!     end do
